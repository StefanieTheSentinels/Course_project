...
demo.py
-------
End-to-end walkthrough of ab_blackbox on synthetic data:
model inspection → single call → A/B test → calibration → power sweep.
"""
...
def main() -> None:
    ...
    print(f"\nRequired sample size per group (80% power, alpha=0.05): {n_required}")
    ...
    # noise_std=0.01 here models between-experiment variation for demo purposes
    ...
    print("\n" + "=" * 60)
    print("4. CALIBRATION FROM SYNTHETIC DATASET")
    print("=" * 60)
    ...
    cal_box    = BlackBox(model=cal["model"], n_users=1000, noise_std=0.0, seed=7)
    ...
    for n in sample_sizes:
        # noise_std=0.01 for power sweep to model realistic between-run variation
